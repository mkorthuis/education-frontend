...
const SuspensionCategoryDetails: React.FC = () => {
    ...
    const currentSchoolDisciplineData = schoolDisciplineCountData.filter(item => 
    ...
    // Extract counts
    const inSchoolSuspensionsCount = currentSchoolDisciplineData.find(item => 
        item.count_type.id === inSchoolSuspensionType?.id)?.count || 0;
    const outOfSchoolSuspensionsCount = currentSchoolDisciplineData.find(item => 
        item.count_type.id === outOfSchoolSuspensionType?.id)?.count || 0;
    const expulsionsCount = currentSchoolDisciplineData.find(item => 
    ...
    // Enrollment data
    const schoolEnrollment = schoolEnrollmentData.find(item => 
    ...
    const SuspensionSummaryContent = ({ asList = true }) => {
        ...
        return (
            <Box component={Container} sx={containerStyles}>
                ...
                <Typography component="li" variant="body1">
                    {schoolSuspensionsPer100.toFixed(0)} Suspensions Per 100 Students at {school?.name}
                </Typography>

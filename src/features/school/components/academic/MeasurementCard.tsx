...
const MeasurementCard: React.FC<MeasurementCardProps> = ({
  ...
  useEffect(() => {
    if(schoolAssessmentData.length === 0 && !isSchoolDataLoading) {
      dispatch(fetchAssessmentSchoolData(queryParams));
    }
  }, [dispatch, schoolAssessmentData, queryParams, isSchoolDataLoading]);
  ...
  const handleCardClick = () => {
    if (id) {
      ...
      const path = PAGE_REGISTRY.school.academic.urlPatterns[0]
        .replace(':id', id)
  ...
  const assessmentData = filteredSchoolData[0];
  ...
  const { rank, total } = useMemo(() => {
    return getSchoolRankInfo(schoolAssessmentData, currentSchoolId);
  }, [schoolAssessmentData, currentSchoolId]);

  const previousYearData = filterAssessmentResults(schoolData, {
            ...
            <>
              ...
              <Box>
                <Typography variant="body2">
                  {rank !== null && total > 0 ? `Ranked #${rank} out of ${total} schools` : ''}
                </Typography>
                <Typography variant="body2" color="text.secondary" sx={{ fontStyle: 'italic' }}>

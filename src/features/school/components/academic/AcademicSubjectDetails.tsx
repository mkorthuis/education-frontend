...
const AcademicSubjectDetails: React.FC<AcademicSubjectDetailsProps> = ({ subject }) => {
  ...
  // Get assessment school data from Redux store
  ...
  return (
    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, mb: 3, gap: 2 }}>
        ...
        <Box sx={{ flex: 1 }}>
          {selectedSubjectId && <ProficiencyByLevelTable schoolName={filteredData[0]?.school_name} />}
        </Box>
      </Box>
      
      {/* Second row: AcademicHistoryChart (on left) and SchoolAcademicPerformance side by side using flex */}
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
        <Box sx={{ flex: 1 }}>
          {selectedSubjectId && <AcademicHistoryChart />}
        </Box>
        <Box sx={{ flex: 1 }}>
          {selectedSubjectId && <SchoolAcademicPerformance />}
        </Box>
      </Box>
    </Box>
  );

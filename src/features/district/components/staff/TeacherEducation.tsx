...
import { TEACHER_EDUCATION_ORDER } from '@/features/district/utils/staffDataProcessing';
...
const TeacherEducation: React.FC = () => {
  ...
  // Get teacher education data
  const districtTeacherEducationData = useAppSelector(state => 
    staffSlice.selectDistrictTeacherEducationData(state, districtParams));
  const stateTeacherEducationData = useAppSelector(state => 
    staffSlice.selectStateTeacherEducationData(state, stateParams));
  const teacherEducationTypes = useAppSelector(staffSlice.selectTeacherEducationTypes);

  // Get available years for comparison, excluding current fiscal year
  const availableYears = useMemo(() => {
    const years = new Set<number>();
    districtTeacherEducationData.forEach(item => {
      if (item.year < defaultFiscalYear) {
        years.add(item.year);
      }
    });
    return Array.from(years).sort((a, b) => b - a);
  }, [districtTeacherEducationData, defaultFiscalYear]);
  ...
  const tableData = useMemo(() => {
    ...
    const districtTotals = districtTeacherEducationData.reduce((acc, item) => {
      if (!acc[item.year]) {
        acc[item.year] = 0;
      }
    ...
    const stateTotals = stateTeacherEducationData.reduce((acc, item) => {
      if (!acc[item.year]) {
        acc[item.year] = 0;
      }
      acc[item.year] += item.value;
    ...
    // Create rows for each teacher education type
    const data = teacherEducationTypes.map(type => {
      const currentItem = districtTeacherEducationData.find(
        item => item.year === currentYear && 
        item.teacher_type.id === type.id
      );
      const comparisonItem = viewMode === 'comparison' 
        ? districtTeacherEducationData.find(
            item => item.year === compareYear && 
            item.teacher_type.id === type.id
          )
        : stateTeacherEducationData.find(
            item => item.year === currentYear && 
            item.teacher_type.id === type.id
          );
    ...
    // Sort the data according to TEACHER_EDUCATION_ORDER
    return data.sort((a, b) => {
      const aIndex = TEACHER_EDUCATION_ORDER.indexOf(a.name as any);
      const bIndex = TEACHER_EDUCATION_ORDER.indexOf(b.name as any);
      return aIndex - bIndex;
    });
  }, [districtTeacherEducationData, stateTeacherEducationData, teacherEducationTypes, defaultFiscalYear, comparisonYear, viewMode]);
